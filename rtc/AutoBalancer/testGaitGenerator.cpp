...
private:
    ...
    void plot_walk_pattern ()
    {
        ...
        while ( gg->proc_one_tick() ) {
            ...
            // Foot pos
            hrp::Vector3 rfoot_pos = (gg->get_support_leg_list_kuro() == boost::assign::list_of("rleg")) ? gg->get_support_leg_coords_list_kuro().front().pos : gg->get_swing_leg_coords_list_kuro().front().pos;
            for (size_t ii = 0; ii < 3; ii++) {
            ...
            hrp::Vector3 lfoot_pos = (gg->get_support_leg_list_kuro() == boost::assign::list_of("lleg")) ? gg->get_support_leg_coords_list_kuro().front().pos : gg->get_swing_leg_coords_list_kuro().front().pos;
            for (size_t ii = 0; ii < 3; ii++) {
            ...
            // Foot rot
            hrp::Vector3 rpy;
            hrp::Matrix33 rfoot_rot = (gg->get_support_leg_list_kuro() == boost::assign::list_of("rleg")) ? gg->get_support_leg_coords_list_kuro().front().rot : gg->get_swing_leg_coords_list_kuro().front().rot;
            rpy = hrp::rpyFromRot(rfoot_rot);
            for (size_t ii = 0; ii < 3; ii++) {
                fprintf(fp, "%f ", 180.0*rpy(ii)/M_PI);
            }
            hrp::Matrix33 lfoot_rot = (gg->get_support_leg_list_kuro() == boost::assign::list_of("lleg")) ? gg->get_support_leg_coords_list_kuro().front().rot : gg->get_swing_leg_coords_list_kuro().front().rot;
            ...
            // ZMP offsets
            for (size_t ii = 0; ii < 3; ii++) {
                fprintf(fp, "%f ", (gg->get_support_leg_list_kuro() == boost::assign::list_of("rleg")) ? gg->get_support_foot_zmp_offset_list_kuro().front()(ii) : gg->get_swing_foot_zmp_offset_list_kuro().front()(ii));
            }
            for (size_t ii = 0; ii < 3; ii++) {
                fprintf(fp, "%f ", (gg->get_support_leg_list_kuro() == boost::assign::list_of("lleg")) ? gg->get_support_foot_zmp_offset_list_kuro().front()(ii) : gg->get_swing_foot_zmp_offset_list_kuro().front()(ii));
            }
            // Swing time
            fprintf(fp, "%f %f ",
    ...
    {
        coordinates initial_support_leg_coords(gg->get_footstep_front_leg_list_kuro()==boost::assign::list_of("rleg")?leg_pos[1]:leg_pos[0]);
        coordinates initial_swing_leg_dst_coords(gg->get_footstep_front_leg_list_kuro()!=boost::assign::list_of("rleg")?leg_pos[1]:leg_pos[0]);
...
public:
    ...
    void test0 ()
    {
        ...
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(100*1e-3, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(200*1e-3, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(200*1e-3, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
    ...
    void test1 ()
    {
        ...
        /* initialize sample footstep_list */
        parse_params();
        gg->clear_footstep_node_list_list_kuro();
        gg->go_pos_param_2_footstep_list_list_kuro(200*1e-3, 100*1e-3, 20, coordinates(leg_pos[0]), coordinates(leg_pos[1]), RLEG);
        gen_and_plot_walk_pattern();
    };
    ...
    void test2 ()
    {
        ...
        parse_params();
        gg->clear_footstep_node_list_list_kuro();
        gg->go_pos_param_2_footstep_list_list_kuro(300*1e-3, 0, 0, coordinates(leg_pos[1]), coordinates(leg_pos[0]), LLEG);
        gen_and_plot_walk_pattern();
    };
    ...
    void test3 ()
    {
        ...
        parse_params();
        gg->clear_footstep_node_list_list_kuro();
        gg->go_pos_param_2_footstep_list_list_kuro(0, 150*1e-3, 0, coordinates(leg_pos[0]), coordinates(leg_pos[1]), RLEG);
        gen_and_plot_walk_pattern();
    };
    ...
    void test4 ()
    {
        ...
        parse_params();
        gg->clear_footstep_node_list_list_kuro();
        gg->go_pos_param_2_footstep_list_list_kuro(0, 0, 30, coordinates(leg_pos[1]), coordinates(leg_pos[0]), LLEG);
        gen_and_plot_walk_pattern();
    };
    ...
    void test5 ()
    {
        ...
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(100*1e-3, 0, 100*1e-3)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(200*1e-3, 0, 200*1e-3)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(300*1e-3, 0, 300*1e-3)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(300*1e-3, 0, 300*1e-3)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        gg->set_foot_steps_list_kuro(fnll_kuro);
        gen_and_plot_walk_pattern();
    };
    ...
    void test6 ()
    {
        ...
        parse_params();
        gg->clear_footstep_node_list_list_kuro();
        gg->go_single_step_param_2_footstep_list_list_kuro(100*1e-3, 0, 0, 0, "rleg", coordinates(leg_pos[0]));
        gen_and_plot_walk_pattern();
    };
    ...
    void test7 ()
    {
        ...
        gg->clear_footstep_node_list_list_kuro();
        gg->go_pos_param_2_footstep_list_list_kuro(100*1e-3, 0, 0, coordinates(leg_pos[1]), coordinates(leg_pos[0]), LLEG);
        gen_and_plot_walk_pattern();
    };
    ...
    void test8 ()
    {
        ...
        gg->go_pos_param_2_footstep_list_list_kuro(100*1e-3, 0, 0, coordinates(initial_foot_mid_rot*leg_pos[1], initial_foot_mid_rot), coordinates(initial_foot_mid_rot*leg_pos[0], initial_foot_mid_rot), LLEG);
        coordinates initial_support_leg_coords(hrp::Vector3(initial_foot_mid_rot * (gg->get_footstep_front_leg_list_kuro()==boost::assign::list_of("rleg")?leg_pos[1]:leg_pos[0])), initial_foot_mid_rot);
        coordinates initial_swing_leg_dst_coords(hrp::Vector3(initial_foot_mid_rot * (gg->get_footstep_front_leg_list_kuro()!=boost::assign::list_of("rleg")?leg_pos[1]:leg_pos[0])), initial_foot_mid_rot);
        gen_and_plot_walk_pattern(initial_support_leg_coords, initial_swing_leg_dst_coords);
    ...
    void test9 ()
    {
        ...
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(250*1e-3, 0, 200*1e-3)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(250*1e-3, 0, 200*1e-3)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(500*1e-3, 0, 400*1e-3)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(500*1e-3, 0, 400*1e-3)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(750*1e-3, 0, 600*1e-3)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(750*1e-3, 0, 600*1e-3)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        gg->set_foot_steps_list_kuro(fnll_kuro);
        gen_and_plot_walk_pattern();
    };
    ...
    void test11 ()
    {
        ...
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(250*1e-3, 0, 0*1e-3)+leg_pos[0]), tmpr), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        tmpr = hrp::rotFromRpy(-5*M_PI/180.0, -15*M_PI/180.0, 0);
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(250*1e-3, 0, 0*1e-3)+leg_pos[1]), tmpr), gg->get_default_step_height(), gg->get_default_step_time(), gg->get_toe_angle(), gg->get_heel_angle())));
        gg->set_foot_steps_list_kuro(fnll_kuro);
        gen_and_plot_walk_pattern();
    };
    ...
    void test12 ()
    {
        ...
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[0])), gg->get_default_step_height(), 1.0, 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[1])), gg->get_default_step_height(), 2.0, 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(100*1e-3, 0, 0)+leg_pos[0])), gg->get_default_step_height()*2, 1.5, 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(200*1e-3, 0, 0)+leg_pos[1])), gg->get_default_step_height(), 2.5, 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(300*1e-3, 0, 0)+leg_pos[0])), gg->get_default_step_height(), 1.0, 20, 5)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(300*1e-3, 0, 0)+leg_pos[1])), gg->get_default_step_height(), 2.0, 0, 0)));
        gg->set_foot_steps_list_kuro(fnll_kuro);
        gen_and_plot_walk_pattern();
    };
    ...
    void test13 ()
    {
        ...
        parse_params();
        std::vector< std::vector<step_node> > fnll_kuro;
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(0, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(100*1e-3, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(100*1e-3, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("rleg", coordinates(hrp::Vector3(hrp::Vector3(200*1e-3, 0, 0)+leg_pos[0])), gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
        fnll_kuro.push_back(boost::assign::list_of(step_node("lleg", coordinates(hrp::Vector3(hrp::Vector3(200*1e-3, 0, 0)+leg_pos[1])), gg->get_default_step_height(), gg->get_default_step_time(), 0, 0)));
        gg->set_foot_steps_list_kuro(fnll_kuro);
        gen_and_plot_walk_pattern();
    };
