...
namespace rats
{
  ...
  protected:
    ...
    std::deque<double> pz;
    std::deque< std::vector<hrp::Vector3> > qdata; /* preview_controllerの引数にswing_zmp_offsetがあるのは，かかとを使った歩行のとき用で，まわりまわってqdataにその値は保持される．難しいことは考えなくてよくて，ここの型をvectorに変えれば良い */
    double zmp_z, cog_z;
